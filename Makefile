...
all: specs/cells/key_bound_ownership_lock specs/cells/simple_otx specs/cells/ckb_cell_upgrade specs/cells/secp256k1_blake160_sighash_all
...
specs/cells/key_bound_ownership_lock: c/key_bound_ownership_lock.c ${PROTOCOL_HEADER}
	$(CC) $(CFLAGS) $(LDFLAGS) -o $@ $<
	$(OBJCOPY) --only-keep-debug $@ $(subst specs/cells,build,$@.debug)
	$(OBJCOPY) --strip-debug --strip-all $@

specs/cells/simple_otx: c/simple_otx.c ${PROTOCOL_HEADER} c/keccak256.h c/common.h c/utils.h build/secp256k1_data_info.h $(SECP256K1_SRC)
	$(CC) $(CFLAGS) $(LDFLAGS) -o $@ $<
	$(OBJCOPY) --only-keep-debug $@ $(subst specs/cells,build,$@.debug)
	$(OBJCOPY) --strip-debug --strip-all $@
...
clean:
	...
	rm -rf specs/cells/key_bound_ownership_lock specs/cells/simple_otx specs/cells/ckb_cell_upgrade specs/cells/secp256k1_blake160_sighash_all
